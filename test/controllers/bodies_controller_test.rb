...
class BodiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create body" do
    assert_difference('Body.count') do
      post bodies_url, params: { body: { recorded_at: @body.recorded_at, weight: @body.weight } }
    end
  ...
  test "should update body" do
    patch body_url(@body), params: { body: { recorded_at: @body.recorded_at, weight: @body.weight } }
